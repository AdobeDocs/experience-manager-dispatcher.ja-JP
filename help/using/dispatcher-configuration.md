...
このようなメッセージが生成される可能性があるのは、`EINTR` が `read more data` セクションで発生する場合です。 問題は、データの前にシグナルが到達することにより発生します。
...
>
>ただし、このようなレベルにはより多くのリソースが必要です。 Dispatcher が&#x200B;*現在の要件に従って*&#x200B;円滑に動作している場合は、ログレベルを低くしてください。
