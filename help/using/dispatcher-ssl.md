...
ユーザーが CA の役目を果たす場合は、[OpenSSL](https://www.openssl.org/) を使用して、サーバーとクライアントの証明書に署名する証明機関を作成します。 （OpenSSL ライブラリがインストールされている必要があります）。 サードパーティ CAを使用している場合は、この手順を実行しないでください。
