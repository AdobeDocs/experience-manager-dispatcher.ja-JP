...
description: AEM Cloud Serviceのセキュリティやキャッシュを向上させるために、Adobe Experience Manager Dispatcher を使用する方法を説明します。
...
>
>ただし、Dispatcher を使用して **オーサーインスタンス**. これは、特に多数のユーザーが web サイトを編集および更新する場合に当てはまります。 このケースについて詳しくは、以下の[オーサーサーバーでの Dispatcher の使用](#using-a-dispatcher-with-an-author-server)を参照してください。
...
このプロセスは簡単で効率的です。訪問者が HTML ページなどのファイルをリクエストすると、ファイルはメモリから直接取得され、最悪の場合でもローカルドライブから読み取られます。静的 web サーバーはかなり前から利用されています。 そのため、管理とセキュリティ管理のためのツールは幅広い。 これらのツールは、ネットワークインフラストラクチャと十分に統合されています。
...
**キャッシュディレクトリ**：キャッシュ時、Dispatcher モジュールでは Web サーバーの静的コンテンツ提供機能を使用します。Dispatcher は、キャッシュしたドキュメントを web サーバーのルートに配置します。
...
>
>ファイル名の長さには OS レベルの制限がある場合があります。 つまり、多数のセレクターを含む URL がある場合です。
...
1. 同じハンドルで始まるすべてのファイルを、キャッシュから削除します。例えば、ファイルが `/en/index.html` が更新され、で始まるすべてのファイルが `/en/index.` が削除されました。 このメカニズムにより、特に画像ナビゲーション用に、キャッシュ効率の高いサイトを設計できます。
...
* リクエスト URI に疑問符が含まれています `?`. このシナリオは通常、キャッシュの必要がない、検索結果などの動的ページを指します。
...
>[!CAUTION]
>
>1 つの Dispatcher で、使用可能なパブリッシュインスタンスの処理能力を飽和させることができますが、まれに、2 つの Dispatcher インスタンス間で負荷のバランスを取る必要が生じる場合があります。 複数の Dispatcher を持つ設定は、慎重に検討する必要があります。 理由は、追加の Dispatcher により、使用可能なパブリッシュインスタンスの負荷が増え、ほとんどのアプリケーションでパフォーマンスが低下しやすくなるからです。
...
スティッキー接続では、Dispatcher でリクエストを最適化する機能が制限されるので、このアプローチはは必要な場合にのみ使用してください。「スティッキー」ドキュメントを含むフォルダーを指定すると、そのフォルダー内のすべてのドキュメントが各ユーザーの同じインスタンスで作成されるようになります。
...
CDN は、HTTP インフラストラクチャコンポーネントとして、Dispatcher とほぼ同じように機能します。 CDN ノードがリクエストを受け取ると、可能な場合（リソースはキャッシュ内にあって有効な場合）はキャッシュからリクエストを提供します。それ以外の場合は、次に近いサーバーに問い合わせてリソースを取得し、適切であれば、今後のリクエストに備えてキャッシュします。
...
通常のAEM設定では、拡張子、パスまたはその両方で設定します（上記の 1 と 2 を通じて実現できます）。これにより、適切なキャッシュ時間を設定できる可能性があります。 これらのキャッシュ期間は、デザインイメージやクライアントライブラリなど、頻繁に変更されない頻繁に使用されるリソース用です。 新しいリリースがデプロイされると、一般的には手動での無効化が必要になります。

このアプローチを使用して管理コンテンツをキャッシュする場合は、設定されたキャッシュ期間が経過した後にのみ、コンテンツの変更がエンドユーザーに表示されることを意味します。 さらに、ドキュメントが Dispatcher から再度取得された場合も同様です。
...
1. Dispatcher が TCP/IP 経由でオーサーインスタンスに接続できることを確認します。
...
1. 内の既存のファイルをすべて削除 `/cache` > `/docroot` 上記で設定したディレクトリ。
1. Web サーバーを再起動します。

>[!NOTE]
>
>が提供されます `author_dispatcher.any` 設定：の下のいずれかのコンテンツに影響を与える CQ5 機能パック、ホットフィックスまたはアプリケーションコードパッケージをインストールした場合 `/libs` または `/apps`は、キャッシュされたファイルを削除する必要があります。 ファイルは、Dispatcher キャッシュ内のこれらのディレクトリの下にあります。 これにより、ファイルが次回リクエストされたときに、キャッシュされた古いファイルではなく、新しくアップグレードされたファイルが取得されるようになります。
